...
@Injectable()
export class GetClinicalRecordByPatientUseCase {
  constructor(
    @Inject(CLINICAL_RECORD_REPOSITORY) private readonly clinicalRecordRepository: ClinicalRecordRepository,
  ) {}

  async execute(patientId: number): Promise<ClinicalRecord> {
    const record = await this.clinicalRecordRepository.findByPatientId(patientId);
    if (!record) throw new ClinicalRecordNotFoundException(`paciente ${patientId}`);
    return record;
  }
}
